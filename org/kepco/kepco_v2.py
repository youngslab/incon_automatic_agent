...
class Kepco(am.Automatic):
    ...
    def register(self, number):
        ...
        if cnt == 0:
            ...
            return False
        if cnt > 1:
            logger.error(f"검색 결과가 한개 초과 입니다. cnt={cnt}")
            return False
    ...
    def close_messagebox(self, button, timeout=1):
        logger.info(f"메세지 박스 닫기: {button}")
        ...
        if self.exist(text):
            logger.info(f"Messagebox: {' '.join(self.text(text).splitlines())}" )
    ...
    def participate(self, code, cost):
        ...
        if table[1].loc[0] != "미제출":
            logger.info(f"이미 등록완료 되었습니다. {table}")
            return True
