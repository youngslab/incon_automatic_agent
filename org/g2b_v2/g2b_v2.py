...
class G2B_v2(am.Automatic):
    def __init__(self, driver, pw, id, loglevel=logging.INFO):
        ...
        super().__init__([selenium, win32])
        self.logger = logging.getLogger("G2B")
    ...
    def certificate(self, pw):
        ...
        self.type(w.Image('패스워드 입력상자', resmgr.get('certificate_password_input.png'), parent=wMain), pw)
        self.click(w.Image('인증서 확인 버튼', resmgr.get('certificate_password_confirm_button.png'), parent=wMain))
    ...
    def login(self):
        ...
        try:
            ...
            self.click(s.Xpath("로그인버튼",'//*[@id="logout"]/ul/li[1]/ul/li/a/img' ,parent=fLogin))
            ...
            click = lambda: self.click(s.Xpath('확인버튼','//span[text()="확인"]/..', parent=fSub))
            ...
            self.click(s.Xpath("확인버튼",  "//span[text()='확인']/..", parent=wConfirm))
    ...
    def register(self, code):
        ...
        try:
            ...
            self.click(s.Xpath("에디트 버트",'//*[@id="container"]/div[2]/div/a[1]', parent=fMain))

            table = self.table(s.Xpath("공급품목표", '//*[@id="frm_supProd"]/div[3]/table', parent=fMain))
            if table['세부품명번호'].isin([int(code)]).any():
                self.logger.info(f"이미 등록되었습니다.")
                return
            
            self.logger.info(f"물품품목 조회")
            self.click(s.Xpath("물품검색버튼", '//*[@id="frm_addProd"]/div[3]/table/tbody/tr/td[1]/div/button', parent=fMain))
            ...
            self.click(s.Xpath("첫번째품목",'//*[@id="container"]/div[1]/table/tbody/tr/td[2]/a', parent=wSearch))

            self.click(s.Xpath("등록버튼",'//*[@id="frm_addProd"]/div[2]/a', parent=fMain))
            ...
            if self.exist(wConfirm):
                self.click(s.Xpath("확인버튼", '//*[@id="container3"]/div[2]/div/a', parent=wConfirm))
    ...
    def participate(self, code, price):
        try:
            ...
            self.click(s.Xpath("지문투찰버튼", "//button[@title='지문투찰']", parent=fMain))
            ...
            if self.exist(s.Xpath("접수완료 메세지", "//div[contains(text(), '접수되었습니다.')]", parent=wMessage)):
                self.click(s.Xpath("닫기버튼", '//*[@id="container3"]/div[2]/div/a/span', parent=wMessage))
            ...
            self.click(s.Xpath("확인버튼", "//span[text()='확인']/..", parent=wNotice))
            ...
            self.type(s.Xpath("임시 입력박스", "//*[@id='attachFile']/tbody/tr/td[2]/input", parent=wBid), "")
            self.click(s.Id("청렴 계약 동의 체크확인 버튼", "checkCleanContract", parent=wBid))
            ...
            self.clicks(s.Name("추첨번호 체크박스", "check", parent=wLottery), num_samples=2)
            # XXX: 너무 빠르게 지나가 버려 확인하기가 어렵다.
            time.sleep(3)
            self.click(s.Xpath("확인 버튼", '//span[text()="추첨번호전송"]/..', parent=wLottery))
            ...
            self.click(w.Image(resmgr.get('safeg2b_2_9_confirm_button.png'), parent=wConfirm))

            self.logger.info("전자입찰 송수신")
            wHistory = s.Title("전자입찰 송수신상세이력조회 창", "전자입찰 송수신상세이력조회")
            self.clicks(s.Xpath("닫기 버튼", "//span[text()='닫기']/..", parent=wHistory))
